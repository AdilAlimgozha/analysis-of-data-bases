...
AS
BEGIN
	SELECT cust_name/*, vend_state, cust_state,
	CASE
		WHEN cust_state = vend_state
		THEN 'YES'
		ELSE 'NO'
	END AS VEND*/
	FROM Customers C
...
BEGIN
	...
	SELECT MAX(MONEY) FROM(
		...
		ON O.order_num = OI.order_num
		GROUP BY C.cust_id 
		ORDER BY SUM(OI.item_price * OI.quantity)) AS MONEY_T) 
	GROUP BY MONEY
	WHERE MONEY NOT IN (SELECT MAX(MONEY) FROM(
		...
		ON O.order_num = OI.order_num
		GROUP BY C.cust_id 
		ORDER BY SUM(OI.item_price * OI.quantity) DESC) AS MONEY_T) )

	SELECT C.cust_id, COUNT(O.order_num) AS NUM_ITEMS, SUM(OI.item_price * OI.quantity) AS MONEY,
	CASE
		WHEN SUM(OI.item_price * OI.quantity) = MAX(SUM(OI.item_price * OI.quantity))
		THEN 'MAXIMUM'
		WHEN SUM(OI.item_price * OI.quantity) = 
